...
*  Date: 2015-08-24 
*/ 
angular.module('template-vstabs-0.0.3.html', []).run(['$templateCache', function($templateCache) {
  $templateCache.put("templates/vstabpage.html",
    "<div ng-show=visible ng-transclude></div>");
  $templateCache.put("templates/vstabs.html",
    "<div class=vstabs><div class=vstabpages><div class=vstabpage ng-repeat=\"p in pages\" ng-style=\"{'width': 100/pages.length + '%'}\" ng-class={vstabactive:p.visible} ng-click=tabClicked(p,$index) ng-keydown=keyDown($event,p,$index) tabindex=0>{{p.title}}</div></div><div class=vstabcontent ng-transclude></div></div>");
}]);
 ...
 */
angular.module('vstabs', ["template-vstabs-0.0.3.html"])
